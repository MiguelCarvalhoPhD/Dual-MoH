...
class RealMLPMethod(Method):
    ...
    def data_format(self, is_train = True, N = None, C = None, y = None):
        if is_train:
            ...
            if not self.is_regression:
                ...
                classes = np.unique(self.y['train'])
                class_weights = compute_class_weight(class_weight='balanced', classes=classes, y=self.y['train'])
                print(f"Class weights: {class_weights}")
                class_weights = torch.tensor(class_weights, dtype=torch.float64).to(self.args.device)
            ...
            if self.is_regression:
                ...
                self.criterion = (partial(F.cross_entropy, weight=class_weights))
    ...
    def fit(self, data, info, train = True, config = None):
        ...
        # chenge criterion to weighted loss based on class distribution
        # if not self.is_regression:
        #     from sklearn.utils.class_weight import compute_class_weight
        #     classes = np.unique(y['train'])
        #     class_weights = compute_class_weight(class_weight='balanced', classes=classes, y=y['train'])
        #     class_weights = torch.tensor(class_weights, dtype=torch.float64).to(self.args.device)
        #     if self.args.use_float:
        #         class_weights = class_weights.float()
        #     else:
    ...
    def predict(self, data, info, model_name):
        ...
        test_logit = torch.from_numpy(test_logit).to(self.args.device, dtype=float)
        test_label = torch.from_numpy(test_label).to(self.args.device, dtype=torch.long)
