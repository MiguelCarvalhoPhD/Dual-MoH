...
def data_loader_process(is_regression, X, Y, y_info, device, batch_size, is_train,is_float = False):
    ...
    #compute class weights for imbalanced classification (note, since )
    if not is_regression:
        print(f"Y keys: {Y.keys()}")
        from sklearn.utils.class_weight import compute_class_weight
        y_train_numpy = Y['train'].cpu().numpy() if is_train else Y['test'].cpu().numpy()
        classes = np.unique(y_train_numpy)
        class_weights = compute_class_weight(class_weight='balanced', classes=classes, y=y_train_numpy)
        class_weights_tensor = torch.tensor(class_weights, dtype=torch.float64).to(device)
    ...
    if is_regression:
        ...
        loss_fn = partial(F.cross_entropy, weight=class_weights_tensor)
