...
os.sys.path.append(osp.abspath(osp.join(osp.dirname(__file__), "..", "..")))
...
class DeepClassifier(BaseEstimator, ClassifierMixin):
    def __init__(
        ...
        model_path=None,
        use_float=None,
        talent_path="/home/miguel/Desktop/TALENT/TALENT/"
    ):
        """
        ...
        if self.model_type in ["tabpfn","tabpfn_v2","hyperfast"]:
    ...
    def fit(self, X, y, categorical_indicator,cost_sensitve=False):
        """
        ...
        #print(f"config used for training: {self.config['model']}")
        print(f"Config used for training: {self.config}")
        
        if cost_sensitve and self.model_type in ["LogReg","RandomForest","lightgbm","svm"]:
            self.config["model"]["class_weight"] = "balanced"
            
        if cost_sensitve and self.model_type == "xgboost":
            from sklearn.utils.class_weight import compute_sample_weight
            sample_weights = compute_sample_weight(class_weight="balanced", y=train_data[2]["train"])
            
            self.config["fit"]["sample_weight"] = sample_weights
        
            #print(f"Config used for training after cs: {self.config['model']}")
    ...
    def predict(self, X):
        ...
        if len(np.shape(predict_logits)) == 2:
            ...
            if isinstance(predict_logits, torch.Tensor):
                prediction = np.argmax(predict_logits.detach().cpu().numpy(), axis=1)
            else:
...
if __name__ == "__main__":
    ...
    #print files in current directory/configs
    #print("Files in current directory/configs:", os.listdir("configs"))
