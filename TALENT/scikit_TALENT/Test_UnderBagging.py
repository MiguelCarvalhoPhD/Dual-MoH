...
def test_deep_classifiers(X_train, y_train, X_test, y_test, categorical_indicator, to_resample=False, resampler=None, to_ordinal_encode=False, to_time=False):
    ...
    for i, model in enumerate(models):
        ...
        clf = UnderSamplingBaggingClassifier(base_estimator=DeepClassifier(model_type=model))
...
def icl_models(X_train, y_train, X_test, y_test, categorical_indicator):
    ...
    icl_models = [
        ...
        ("EnsembleMixtutre", CustomEnsembleClassifier(n_estimators=10, base_model_class=Dual_MoH, model_params={"m":6, "overlap":0.15, "verbose":False, "random_state":10, "minority_cluster":'KMeans', "majority_cluster":'BalancedKMeansLSA'}, mode="regular")),
    ...
    for i, (model_name, model) in enumerate(icl_models):
        ...
        if model_name == "MOH" or model_name == "MOH_mothernet":
            ...
            if model_name == "EnsembleMixtutre":
                ...
                model_bag = UnderSamplingBaggingClassifier(base_estimator=model)
                model_bag.fit(X_icl, y_icl)
                torch.cuda.synchronize()
                time_end = time.time()
        ...
        for rs_idx, rs in enumerate(random_states):
            try:
                ...
                with SuppressStdoutStderr():
                    res_d, _, m_names, _ = test_deep_classifiers(X_train, y_train, X_test, y_test, categorical_indicator, to_resample=False, resampler=RandomUnderSampler)
...
results_df.to_csv("Final_benchmark_56_datasest_underbagging_no_swap_no_tree_router.csv", index=False)